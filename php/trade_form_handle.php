...
if ($_GET['confirmed'] == "true") {
	...
	if (false/*!marketIsOpen() || $ticker == -1 || !isValidTicker($ticker) || $amount * getCurrentPrice($ticker) > getBalance($email)*/) {
		...
		echo '<body onload="confirmPurchase()">
				<script type="text/javascript">
					function confirmPurchase() {
						...
						if (confirmed) {
							window.location.href = "trade_form_handle.php?email=' . $email . 'confirmed=true&ticker=' . $ticker . '&amount=' . $amount . '";
						} else {
							window.location.href = "..";
						}
					} </script>';
